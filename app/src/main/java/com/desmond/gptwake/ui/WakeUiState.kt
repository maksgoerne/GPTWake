...
enum class SetupStep { MIC, NOTIFICATIONS, NOTIFICATION_ACCESS, OVERLAY, ASSISTANT, DONE }
...
data class Permissions(
    ...
    val next: SetupStep
        get() = when {
            ...
            !notificationAccess -> SetupStep.NOTIFICATION_ACCESS
    ...
    fun granted(step: SetupStep): Boolean = when (step) {
        ...
        SetupStep.NOTIFICATION_ACCESS -> notificationAccess
...
/** Engine state. 700ms is plenty for text that reads as prose. */
@Composable
...
/** Collapses the engine's states onto the modes the indicator distinguishes. */
